...
- **内嵌密文**: 可选地将 `.pack` 数据写入 MP4 自定义 UUID box，实现单文件分发且保持播放器兼容（可能导致性能下降）。
...
4.  **视频处理流水线 (`scripts/video_pipeline.py`)**:
    ...
    -   处理完成的元数据（如加密后的 ROI、密钥信息等）被写入一个 `.pack` 文件，用于后续的视频恢复；也可选择（GUI 默认）将该数据打包进输出 MP4 的自定义 UUID box 中，便于携带。
